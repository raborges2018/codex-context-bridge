"""Interface Streamlit para gerar contexto compacto local/offline para o Claude App."""
...
from src.intent_router import INTENT_OPTIONS, resolve_intent_query
from src.markdown_writer import save_markdown
from src.searcher import search_chunks
from src.summarizer import build_offline_markdown

st.set_page_config(page_title="codex-context-bridge", layout="wide")
st.title("codex-context-bridge")
st.caption("Ferramenta 100% offline para compactar contexto técnico antes de colar no Claude App.")
        ...
        if not sources:
            ...
            if not cleaned_records:
                ...
                if len(cleaned_text) > MAX_INPUT_CHARS_DIRECT:
                    ...
                    query = resolve_intent_query(intent) + " " + (additional_instruction or "")
                ...
                file_path = save_markdown(summary)
                st.session_state.generated_markdown = summary
